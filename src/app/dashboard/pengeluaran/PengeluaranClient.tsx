...
const COLOR_MAP = [
  ...
  { bg: 'bg-gray-100 dark:bg-gray-700', text: 'text-gray-600 dark:text-gray-400' },
]
...
export default function PengeluaranClient({ telegramId }: PengeluaranClientProps) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/pengeluaran?telegram_id=${telegramId}&tanggal=${tanggal}`)
  ...
  return (
    <div className="space-y-5">
      ...
      {Object.keys(rekapKat).length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {Object.entries(rekapKat).map(([kat, total]) => {
            const label = getKategoriLabel(kat)
            ...
            return (
              ...
              >
                ...
                <p className="text-sm font-bold text-gray-800 dark:text-white/90 mt-1">{fmt(total as number)}</p>
              </button>
            )
          })}
        </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-5 py-3 dark:border-gray-800 dark:bg-gray-900">
        ...
        <span className="text-sm font-bold text-error-600 dark:text-error-400">-{fmt(totalFiltered)}</span>
      </div>
            ...
            return (
              <div key={p.id}
                ...
                <div className="flex-1 min-w-0">
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wide ${color.bg} ${color.text}`}>
                    {label}
                  </span>
                  {p.catatan && (
